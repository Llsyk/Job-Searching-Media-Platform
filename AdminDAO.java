...
public class AdminDAO {
    ...
    public Admin getAdminByDefaultId() {
        return getAdminById(1L);
    }

    public void updateAdminPhoto(long adminId, String fileName) {
        String sql = "UPDATE Admins SET admin_photo = ? WHERE admin_id = ?";
        jdbcTemplate.update(sql, fileName, adminId);
    }

    public void deleteAdminPhoto(long adminId) {
        String defaultPhotoPath = "default_pic.jpg"; // Path to default photo
        String sql = "UPDATE Admins SET admin_photo = ? WHERE admin_id = ?";
